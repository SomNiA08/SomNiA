...
4. **devil 마지막 발언**: panelist 전원 반환 후에만 `devils-advocate`를 단독 실행 →
   ...
   - **반환 수신 (v0.43 필수)**: "panelist 전원 반환"은 **Agent 도구의 반환 수신**을 뜻한다 —
     `rounds/` 파일의 mtime·해시·크기가 안정됐다는 사실은 **완료 신호가 아니다**. 반환 수신 목록(N/N)을
     `log.md`에 남긴다. ⛔ **에이전트의 반환을 기다리지 않고 파일 상태의 안정을 완료 신호로 읽지 마라** —
     **완료 신호 없이 읽은 파일은 산출물이 아니라 중간 상태다** (AGENTS §4 v0.43).
4-1. **걸음 게이트 (역할 경계 기계 검증 · HANDOFF W5)**: 발언 배분 직후 — **배분한 에이전트 전원의 반환을
   수신한 뒤에만** (반환 수신 목록이 log에 없으면 이 걸음을 실행하지 마라: 완료 신호 없이 잰 파일은 중간 상태다) —
   ...
   (v0.43 — 출처: ai-worklog cycle 6, retro/2026-07-12-retro-cycle6.md 실패 2 — 메인 루프가 중간 상태를
   확정 측정으로 두 번 읽었고, 그 오측정이 만든 유령 결함이 한 걸음을 낭비시켰다.)
